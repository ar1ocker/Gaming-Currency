...
class ExchangesConfirmAPITests(TestCase):
    @classmethod
    def setUpTestData(cls) -> None:
        ...
        cls.confirm_reverse_path = reverse("exchanges_confirm")
    ...
    def test_confirm_valid(self):
        ...
        self.assertEqual(self.exchange.status, "CONFIRMED")
        self.assertEqual(self.exchange.status_description, "description for status")

    def test_enforce_confirm(self):
        service = CurrencyServicesTestFactory(
            permissions={
                "exchanges": {
                    "enabled": True,
                    "confirm": {"enabled": False, "services": [self.service.name]},
                }
            }
        )
        ...
        self.assertIn("Confirm is disabled", data.get("message"))
